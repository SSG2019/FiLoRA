...
from filoralib import prepare_glue_dataset, create_glue_dataloaders, select_lora_modules, apply_filora, evaluate, compute_UV_random
...
stats_UV = compute_UV_random(model_classify, layer_names, rank)
#######################################################################################################################
...
# Apply Adapters
##############
model_random = apply_filora(model_classify, stats_UV, rank=rank)
...
##############
for n, p in model_random.named_parameters():
    if n.endswith(".R"):
...
trainable_params = [p for p in model_random.parameters() if p.requires_grad]

print(f"Total parameters to train: {sum(p.numel() for p in trainable_params):,}")
print(f"Parameters in LoRA .R: {sum(p.numel() for n, p in model_random.named_parameters() if n.endswith('.R') and p.requires_grad):,}")
print(f"Parameters in classifier: {sum(p.numel() for n, p in model_random.named_parameters() if 'classifier' in n and p.requires_grad):,}")
...
train_log_file = f"result/train_loss_random.csv"
eval_log_file = f"result/eval_score_random.csv"
global_step = 0
...
for epoch in range(1, epochs + 1):
    ...
    eval_score = evaluate(model_random, val_loader, device, task_name=task_name, epoch=epoch)
